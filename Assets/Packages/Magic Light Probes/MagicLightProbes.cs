...
namespace MagicLightProbes
{
    ...
    public class MagicLightProbes : MonoBehaviour
    {
        ...
        {
            string scenePath = assetEditorPath + "/Scene Data/" + UnityEngine.SceneManagement.SceneManager.GetActiveScene().name;
        ...
        public void ConfigureInnerVolumes()
        {
            innerVolumes.Clear();
        ...
        private List<GameObject> PrioritizeVolumeList(List<GameObject> volumes)
        {
            ...
            List<GameObject> nonPrioritizedParts = new List<GameObject>();
            ...
            for (int i = 0; i < volumes.Count; i++)
            {
                ...
                {
                    nonPrioritizedParts.Add(volumes[i]);
                }
            }

            sortedVolumeParts.AddRange(prioritizedParts);
            sortedVolumeParts.AddRange(nonPrioritizedParts);
        ...
        private IEnumerator RecalculateLightIntensityRealtime(CullByLightIntensity cullByLightIntensity, float newValue)
        {
            ...
            combinedVolumeComponent.quickEditing = true;
        }
        ...
        private IEnumerator RecalculateFreeFillingRateRealtime(PartialVolumeFilling freeProbesOptimization, float newValue)
        {
            ...
            if (combinedVolumeComponent == null)
            {
                combinedVolumeComponent = GetCombinedVolume(combinedVolumeComponent);
            }
        ...
        public IEnumerator DebugCalculateProbesVolume()
        {
            ...
            GetCombinedVolume(combinedVolumeComponent).combined = false;
            ...
            foreach (var volumePart in volumeParts)
            {
                ...
                switch (debugPass)
                {
                    ...
                    case DebugPasses.GeometryEdges:
                        ...
                        passesToExecute.Add(cullGeometryCollisions.ExecutePass(this, volumePartParameters, CalculationTarget.GeneralCalculation));
                        passesToExecute.Add(checkNearbyGeometry.ExecutePass(this, currentVolumeSpacing, currentCornersDetectionThreshold, currentVolume));
                        passesToExecute.Add(findGeometryEdges.ExecutePass(this, currentVolumeSpacing, currentVolume));
                        break;
        ...
        public IEnumerator CalculateProbesVolume()
        {
            ...
            if (pStage != null)
            {
                if (pStage.prefabContentsRoot.transform.position != Vector3.zero || pStage.prefabContentsRoot.transform.rotation != Quaternion.identity)
                {
#if UNITY_2020_1_OR_NEWER
                    if (pStage.mode == PrefabStage.Mode.InContext)
                    {
                        Debug.LogFormat(
                            "<color=yellow>MLP:</color> Switch to the isolated prefab editing mode to perform the calculation.");
                    }
                    else
                    {
                        Debug.LogFormat(
                            "<color=yellow>MLP:</color> The Position/Rotation/Scale of the prefab root object must be set to (X:0; Y:0; Z:0).");
                    }
#else
                    Debug.LogFormat("<color=yellow>MLP:</color> The Position/Rotation/Scale of the prefab root object must be set to (X:0; Y:0; Z:0).");
#endif
            ...
            var volumes = new List<MagicLightProbes>();

            if (PrefabStageUtility.GetCurrentPrefabStage() != null)
            {
                volumes.AddRange(StageUtility.GetCurrentStageHandle().FindComponentsOfType<MagicLightProbes>());
            }
            else
            {
                volumes.AddRange(FindObjectsOfType<MagicLightProbes>());
            }
            ...
            if (calculatedVolumes == volumes.Count)
            {
                RestoreScene();

                if (PrefabStageUtility.GetCurrentPrefabStage() == null)
                {
                    CombineVolumes(volumes.ToArray());
                }
            }
            else
            {
        ...
        private void GetTransformChilds(GameObject gameObject, ref List<GameObject> childs)
        {
            for (int i = 0; i < gameObject.transform.childCount; i++)
            {
                ...
                {
                    GetTransformChilds(childObject, ref childs);
                }
            }
        }
        ...
        private IEnumerator PrepareScene()
        {
            ...
            if (prefabStage != null)
            {
                ...
                prefabConnectionObject = prefabStage.prefabContentsRoot.GetComponent<MLPPrefab>();
                
                if (prefabConnectionObject == null)
                {
                    prefabConnectionObject = prefabStage.prefabContentsRoot.AddComponent<MLPPrefab>();
                }
                
                prefabPosition = prefabStage.prefabContentsRoot.transform.position;
                prefabRoot = prefabStage.prefabContentsRoot;
                prefabRootName = prefabRoot.name;
                isInPrefab = true;
                calculatedFromPrefab = true;

                if (string.IsNullOrEmpty(prefabUID))
                {
                    string prefabUidString = prefabRoot.name + "_" + gameObject.name;
                    prefabUID = prefabUidString.GetHashCode().ToString();
                }
            }
            else
            {
                calculatedFromPrefab = false;
            }

            finalStaticGameObjectsList = new List<GameObject>();
            ...
            UnityEngine.Object[] All_GOs = new UnityEngine.Object[0];

            if (prefabStage != null)
            {
                List<GameObject> prefabGameObjects = new List<GameObject>();
                GetTransformChilds(prefabStage.prefabContentsRoot, ref prefabGameObjects);

                All_GOs = prefabGameObjects.ToArray();
            }
            else
            {
                All_GOs = FindObjectsOfType(typeof(GameObject));
            }    
...
#else
                if ((flags & StaticEditorFlags.LightmapStatic) != 0 || obj.GetComponent<MeshRenderer>() == null)
                {
                    //if (obj.GetComponents(typeof(Component)).Length > 1)
                    {
                        staticObjects.Add(obj);
            ...
            foreach (var obj in staticObjects)
            {
                if (obj.GetComponent<MLPLight>() == null && obj.GetComponent<MeshFilter>() != null)
                {
                    ...
                    finalStaticGameObjectsList.Add(obj);
                }
        ...
        public bool RaycastPhysicsScene(Vector3 origin, Vector3 direction, float maxDistance, out RaycastHit hitInfoForward, bool raycastAll = false)
        {
            ...
            if (PrefabStageUtility.GetCurrentPrefabStage() != null)
            {
                return prefabStage.scene.GetPhysicsScene().Raycast(
                    origin, 
                    direction, 
                    out hitInfoForward, 
                    distance, 
            ...
            {
                return Physics.Raycast(origin, direction, out hitInfoForward, distance, layerMask);
            }
        }
        ...
        IEnumerator SetProbes(int subVolumeIndex = 0)
        {
            ...
            else
            {
                if (workflow == Workflow.Advanced)
                {
                    string dirPath = assetEditorPath + "/Scene Data/" + UnityEngine.SceneManagement.SceneManager.GetActiveScene().name + "/" + name + "/ContrastPointsData";
        ...
        public MLPCombinedVolume GetCombinedVolume(MLPCombinedVolume combinedVolume)
        {
            if (combinedVolume == null)
            {
                combinedVolume = GameObject.Find("-- MLP Combined Volume --").GetComponent<MLPCombinedVolume>();
            }
        ...
        public void CombineVolumes(MagicLightProbes[] volumes)
        {
            ...
            foreach (var volume in volumes)
            {
                ...
                if (volume.recombinationNeeded)
                {
                    ...
                    if (volume.calculatedFromPrefab)
                    {
                        if (volume.prefabConnectionObject != null)
                        {
                            transformationMatrix = volume.prefabConnectionObject.transform.localToWorldMatrix;
                        }
                        else
                        {
                            Debug.LogFormat(
                                "<color=yellow>MLP:</color> Prefab Connection object lost for ["+ volume.prefabRootName + "], volume recalculation required.");
                        }
                    }

                    foreach (var point in volume.localFinishedPositions)
                    {
                        if (CheckIfInside(volume.transform, point))
                        {
                            continue;
                        }
                        ...
                        if (volume.isInPrefab)
                        {
                            transformedPoint = volume.calculatedFromPrefab ? transformationMatrix.MultiplyPoint3x4(point) : 
                                finalProbeGroup.transform.InverseTransformPoint(point);
                        }
                        else
                        {
                            transformedPoint = finalProbeGroup.transform.InverseTransformPoint(point);
                        }
            ...
            //        {
            //            if (Vector3.Distance(positions[i], positions[j]) >= volumeSpacing)
            //            {
            //                approved++;
            //            }
            //            else
        ...
        public bool CheckIfInside(Transform volumeParameters, Vector3 position)
        {
            float minX = volumeParameters.position.x - (volumeParameters.localScale.x / 2);
            float maxX = volumeParameters.position.x + (volumeParameters.localScale.x / 2);
            float minY = volumeParameters.position.y - (volumeParameters.localScale.y / 2);
            float maxY = volumeParameters.position.y + (volumeParameters.localScale.y / 2);
            float minZ = volumeParameters.position.z - (volumeParameters.localScale.z / 2);
            float maxZ = volumeParameters.position.z + (volumeParameters.localScale.z / 2);
        ...
        public static GameObject CreateHierarchyStructure(PrefabStage currentPrefabStage)
        {
            GameObject magicToolsObject;
            GameObject mlpGroupsObject;

            if (currentPrefabStage != null)
            {
                Transform magicToolsRoot = currentPrefabStage.prefabContentsRoot.transform.Find("Magic Tools [Prefab]");

                magicToolsObject = magicToolsRoot != null ? magicToolsRoot.gameObject : null;

                if (magicToolsObject == null)
                {
                    magicToolsObject = new GameObject("Magic Tools [Prefab]");
                    magicToolsObject.transform.parent = currentPrefabStage.prefabContentsRoot.transform;
                }

                Transform mlpRoot = magicToolsObject.transform.Find("Magic Light Probes");

                mlpGroupsObject = mlpRoot != null ? mlpRoot.gameObject : null;
            ...
            {
                magicToolsObject = GameObject.Find("Magic Tools");
