...
namespace MagicLightProbes
{
    ...
    {       
        ...
        public static GUIStyle toogleButtonActiveStyle;
        public static GUIStyle toogleButtonNormalStyle;
        ...
        private static int lastGroupsCount;
        ...
        public static void InitStyles()
        {
            ...
            toogleButtonNormalStyle = new GUIStyle(GUI.skin.button)
            {
                fixedWidth = 123
            };
            
            toogleButtonActiveStyle = new GUIStyle(toogleButtonNormalStyle);
            toogleButtonActiveStyle.normal.background = toogleButtonActiveStyle.active.background;
        ...
        public static void MLPMainComponentEditor(MagicLightProbes magicLightProbes)
        {
            ...
            if (!magicLightProbes.calculatingVolume)
            {
                ...
                switch (magicLightProbes.selectedTab)
                {
                    case 0:
                        ...
                        GUILayout.Label(MLPTooltipManager.MainComponent.GetParameter("Settings Asset", MLPTooltipManager.MainComponent.Tabs.BasicParameters));
                        GUILayout.FlexibleSpace();
                        magicLightProbes.settingsAsset = (MLPSettings) EditorGUILayout.ObjectField(magicLightProbes.settingsAsset, typeof(MLPSettings), false);

                        if (magicLightProbes.settingsAsset == null)
                        {
                            if (GUILayout.Button("Create", GUILayout.MinWidth(123)))
                            {
                                CreateSettingsAsset(magicLightProbes);
                                SaveSettingAsset(magicLightProbes);
                                magicLightProbes.lastSettingsAsset = magicLightProbes.settingsAsset;
                            }
                        }
                        else
                        {
                            if (magicLightProbes.lastSettingsAsset != magicLightProbes.settingsAsset)
                            {
                                GUI.enabled = false;
                            }
                            
                            if (GUILayout.Button("Auto Save",
                                magicLightProbes.autoSaveSettings ? toogleButtonActiveStyle : toogleButtonNormalStyle))
                            {
                                if (!magicLightProbes.autoSaveSettings)
                                {
                                    magicLightProbes.autoSaveSettings = true;
                                }
                                else
                                {
                                    magicLightProbes.autoSaveSettings = false;
                                }
                            }
                        ...
                        if (magicLightProbes.settingsAsset != null && magicLightProbes.lastSettingsAsset != magicLightProbes.settingsAsset)
                        {
                            magicLightProbes.autoSaveSettings = false;
                            
                            if (GUILayout.Button("Load", GUILayout.MinWidth(123)))
                            {
                                LoadSettingAsset(magicLightProbes);
                                magicLightProbes.lastSettingsAsset = magicLightProbes.settingsAsset;
                            }
                        }
                        else
                        {
                            if (magicLightProbes.autoSaveSettings || magicLightProbes.settingsAsset == null)
                            {
                                GUI.enabled = false;
                            }
                        
                            if (GUILayout.Button("Save", GUILayout.MinWidth(123)))
                            {
                                SaveSettingAsset(magicLightProbes);
                            }
                        ...
                        else
                        {
                            ...
                            magicLightProbes.volumeSpacing = EditorGUILayout.FloatField(magicLightProbes.volumeSpacing, GUILayout.MaxWidth(250));
                        ...
                        else
                        {
                            ...
                            magicLightProbes.cornersDetectionThreshold = EditorGUILayout.Slider(magicLightProbes.cornersDetectionThreshold, 0.1f, magicLightProbes.volumeSpacing, GUILayout.MaxWidth(250));
                        ...
                        GUILayout.Label(MLPTooltipManager.MainComponent.GetParameter("Divide Volume On Probes Count", MLPTooltipManager.MainComponent.Tabs.BasicParameters), GUILayout.MinWidth(150));
                        ...
                        if (!magicLightProbes.useVolumeBottom)
                        {
                            ...
                            if (!magicLightProbes.globalGroundObjects)
                            {
                                for (int i = 0; i < magicLightProbes.allVolumes.Count; i++)
                                {
                                    ...
                                    {
                                        magicLightProbes.storedGroundAndFloorKeywords = new List<string>(magicLightProbes.groundAndFloorObjects);
                                        magicLightProbes.groundAndFloorObjects = new List<string>(magicLightProbes.allVolumes[i].groundAndFloorObjects);
                            ...
                            if (!globalGroundObjectsUsed)
                            {
                                ...
                                if (magicLightProbes.groundAndFloorObjects.Count > 0)
                                {
                                    for (int i = 0; i < magicLightProbes.groundAndFloorObjects.Count; i++)
                                    {
                                        GUILayout.BeginVertical(GUI.skin.box);
                                        GUILayout.BeginHorizontal();

                                        GUILayout.Label(magicLightProbes.groundAndFloorObjects[i], GUILayout.MinWidth(200));

                                        if (GUILayout.Button("Select Object"))
                                        {
                                            Selection.activeGameObject = GameObject.Find(magicLightProbes.groundAndFloorObjects[i]);
                                        }

                                        if (GUILayout.Button("Remove Object"))
                                        {
                                            magicLightProbes.groundAndFloorObjects.Remove(magicLightProbes.groundAndFloorObjects[i]);
                                        }
                                ...
                                if (dragDropArea.Contains(UnityEngine.Event.current.mousePosition))
                                {
                                    ...
                                    else if (UnityEngine.Event.current.type == EventType.DragPerform)
                                    {
                                        for (int i = 0; i < DragAndDrop.objectReferences.Length; i++)
                                        {
                                            ...
                                            {
                                                if (!magicLightProbes.groundAndFloorObjects.Contains(newGroundObject.name))
                                                {
                                                    if (newGroundObject.GetComponent<MeshRenderer>() != null || newGroundObject.GetComponent<Collider>() != null)
                                                    {
                                                        ...
                                                        if (isStatic)
                                                        {
                                                            magicLightProbes.groundAndFloorObjects.Add(newGroundObject.name);
                        ...
                        if (magicLightProbes.tooManySubVolumes)
                        {
                            ...
                            EditorGUILayout.HelpBox("The main volume contains too many sub-volumes. \r\n\r\n" +
                                    "What can you do: \r\n" +
                                    " - Increase the \"Divide Volume On Probes Count\" parameter\r\n" +
                ...
                if (magicLightProbes.isInPrefab && PrefabStageUtility.GetCurrentPrefabStage() == null)
                {
                    if (GUILayout.Button("Edit In Prefab Mode"))
                    {
                        AssetDatabase.OpenAsset(AssetDatabase.LoadAssetAtPath<GameObject>(
                            PrefabUtility.GetPrefabAssetPathOfNearestInstanceRoot(magicLightProbes.prefabRoot)));
                    }
                }
                else
                {
            ...
            if (!magicLightProbes.calculatingVolume)
            {
                if (magicLightProbes.isInPrefab)
                {
                    if (PrefabStageUtility.GetCurrentPrefabStage() != null)
                    {
                        if (GUILayout.Button("Calculate Probes Volume"))
                        {
                            BeginCalculation(magicLightProbes, lockCalculation);
                        }
                    }
                    else
                    {
                        if (GUILayout.Button("Calculate Probes In Prefab Mode"))
                        {
                            AssetDatabase.OpenAsset(AssetDatabase.LoadAssetAtPath<GameObject>(
                                PrefabUtility.GetPrefabAssetPathOfNearestInstanceRoot(magicLightProbes.prefabRoot)));
                        }
        ...
        private static void CreateSettingsAsset(MagicLightProbes magicLightProbes)
        {
            string[] directories = Directory.GetDirectories(Application.dataPath, "Magic Light Probes", SearchOption.AllDirectories);
            string projectRelativePath = directories[0].Split(new[] { "Magic Light Probes" }, StringSplitOptions.None)[0];

            projectRelativePath += "Magic Light Probes/Presets/" + UnityEngine.SceneManagement.SceneManager.GetActiveScene().name + ".asset";

            if (AssetDatabase.LoadAssetAtPath<MLPSettings>(FileUtil.GetProjectRelativePath(projectRelativePath)) != null)
            {
                Debug.Log("Already exist.");
                return;
            }
                
            MLPSettings settingsAsset = Editor.CreateInstance<MLPSettings>();

            AssetDatabase.CreateAsset(settingsAsset, FileUtil.GetProjectRelativePath(projectRelativePath));
            AssetDatabase.SaveAssets();

            magicLightProbes.settingsAsset = (MLPSettings) AssetDatabase.LoadAssetAtPath<MLPSettings>(FileUtil.GetProjectRelativePath(projectRelativePath));
        }
        
        private static void SaveSettingAsset(MagicLightProbes magicLightProbes)
        {
            if (magicLightProbes.settingsAsset == null)
            {
                return;
            ...
            magicLightProbes.settingsAsset.workflow = magicLightProbes.workflow;
            magicLightProbes.settingsAsset.probesSpacing = magicLightProbes.volumeSpacing;
            magicLightProbes.settingsAsset.cornersDetectionThreshold = magicLightProbes.cornersDetectionThreshold;
            magicLightProbes.settingsAsset.cornerProbesSpacing = magicLightProbes.cornerProbesSpacing;
            magicLightProbes.settingsAsset.distanceFromGeometry = magicLightProbes.distanceFromNearbyGeometry;
            magicLightProbes.settingsAsset.divideVolumeOnProbesCount = magicLightProbes.maxProbesInVolume;
            magicLightProbes.settingsAsset.useVolumeBottom = magicLightProbes.useVolumeBottom;
            magicLightProbes.settingsAsset.selectedObjectsAsGlobal = magicLightProbes.globalGroundObjects;
            magicLightProbes.settingsAsset.maxHeightAboveGeometry = magicLightProbes.maxHeightAboveGeometry;
            magicLightProbes.settingsAsset.maxHeightAboveTerrain = magicLightProbes.maxHeightAboveTerrain;
            magicLightProbes.settingsAsset.collisionDetectionLayers = magicLightProbes.layerMask;
            magicLightProbes.settingsAsset.collisionDetectionRadius = magicLightProbes.collisionDetectionRadius;
            magicLightProbes.settingsAsset.fillingMode = magicLightProbes.fillingMode;
            magicLightProbes.settingsAsset.considerDistanceToLights = magicLightProbes.considerDistanceToLights;
            magicLightProbes.settingsAsset.lightIntensityThreshold = magicLightProbes.lightIntensityTreshold;
            magicLightProbes.settingsAsset.cullByColor = magicLightProbes.cullByColor;
            magicLightProbes.settingsAsset.colorThreshold = magicLightProbes.colorTreshold;
            magicLightProbes.settingsAsset.fillEquivalentVolume = magicLightProbes.fillEquivalentVolume;
            magicLightProbes.settingsAsset.equivalentVolumeFillingRate = magicLightProbes.equivalentVolumeFillingRate;
            magicLightProbes.settingsAsset.fillUnlitVolume = magicLightProbes.fillUnlitVolume;
            magicLightProbes.settingsAsset.unlitVolumeFillingRate = magicLightProbes.unlitVolumeFillingRate;
            magicLightProbes.settingsAsset.optimizeForMixedLighting = magicLightProbes.optimizeForMixedLighting;
            magicLightProbes.settingsAsset.tryPreventLightLeakage = magicLightProbes.preventLeakageThroughWalls;
        }
        
        private static void LoadSettingAsset(MagicLightProbes magicLightProbes)
        {
            if (magicLightProbes.settingsAsset == null)
            {
                return;
            ...
            magicLightProbes.workflow = magicLightProbes.settingsAsset.workflow;
            magicLightProbes.volumeSpacing = magicLightProbes.settingsAsset.probesSpacing;
            magicLightProbes.cornersDetectionThreshold = magicLightProbes.settingsAsset.cornersDetectionThreshold;
            magicLightProbes.cornerProbesSpacing = magicLightProbes.settingsAsset.cornerProbesSpacing;
            magicLightProbes.distanceFromNearbyGeometry = magicLightProbes.settingsAsset.distanceFromGeometry;
            magicLightProbes.maxProbesInVolume = magicLightProbes.settingsAsset.divideVolumeOnProbesCount;
            magicLightProbes.useVolumeBottom = magicLightProbes.settingsAsset.useVolumeBottom;
            magicLightProbes.globalGroundObjects = magicLightProbes.settingsAsset.selectedObjectsAsGlobal;
            magicLightProbes.maxHeightAboveGeometry = magicLightProbes.settingsAsset.maxHeightAboveGeometry;
            magicLightProbes.maxHeightAboveTerrain = magicLightProbes.settingsAsset.maxHeightAboveTerrain;
            magicLightProbes.layerMask = magicLightProbes.settingsAsset.collisionDetectionLayers;
            magicLightProbes.collisionDetectionRadius = magicLightProbes.settingsAsset.collisionDetectionRadius;
            magicLightProbes.fillingMode = magicLightProbes.settingsAsset.fillingMode;
            magicLightProbes.considerDistanceToLights = magicLightProbes.settingsAsset.considerDistanceToLights;
            magicLightProbes.lightIntensityTreshold = magicLightProbes.settingsAsset.lightIntensityThreshold;
            magicLightProbes.cullByColor = magicLightProbes.settingsAsset.cullByColor;
            magicLightProbes.colorTreshold = magicLightProbes.settingsAsset.colorThreshold;
            magicLightProbes.fillEquivalentVolume = magicLightProbes.settingsAsset.fillEquivalentVolume;
            magicLightProbes.equivalentVolumeFillingRate = magicLightProbes.settingsAsset.equivalentVolumeFillingRate;
            magicLightProbes.fillUnlitVolume = magicLightProbes.settingsAsset.fillUnlitVolume;
            magicLightProbes.unlitVolumeFillingRate = magicLightProbes.settingsAsset.unlitVolumeFillingRate;
            magicLightProbes.optimizeForMixedLighting = magicLightProbes.settingsAsset.optimizeForMixedLighting;
            magicLightProbes.preventLeakageThroughWalls = magicLightProbes.settingsAsset.tryPreventLightLeakage;
        }

        private static void BeginCalculation(MagicLightProbes magicLightProbes, bool lockCalculation)
        {
            if (!lockCalculation)
            {
                if (magicLightProbes.groundAndFloorObjects.Count == 0 && !magicLightProbes.useVolumeBottom)
                {
                    EditorUtility.DisplayDialog("Magic Light Probes",
                        "Before starting the calculation, the parameter \"Ground And Floor Keywords\" must be configured.",
                        "OK");
                    return;
                }
                ...
                if (magicLightProbes.debugMode)
                {
                    magicLightProbes.lightProbesVolumeCalculatingRoutine =
                        magicLightProbes.DebugCalculateProbesVolume();
                }
                else
                {
                    magicLightProbes.lightProbesVolumeCalculatingRoutine =
                        magicLightProbes.CalculateProbesVolume();
                }
            ...
            {
                EditorUtility.DisplayDialog("Magic Light Probes",
                    "It is not possible to calculate in this mode, " +
            ...
            List<MagicLightProbes> allGroups)
        {
            ...
            List<MagicLightProbes> tempVolumeList = new List<MagicLightProbes>();

            if (currentPrefabStage != lastPrefabStage)
            {
                lastPrefabStage = currentPrefabStage;
                listChanged = true;
            }

            if (allGroups.Count != lastGroupsCount)
            {
                lastGroupsCount = allGroups.Count;
                listChanged = true;
            }
            ...
            if (allGroups.Count == 0 || listChanged)
            {
                ...
                {
                    tempVolumeList.AddRange(StageUtility.GetCurrentStageHandle()
                            .FindComponentsOfType<MagicLightProbes>());

                    for (int i = 0; i < tempVolumeList.Count; i++)
                    {
                        if (tempVolumeList[i].isInPrefab)
                        {
                            tempVolumeList[i].prefabRootName = currentPrefabStage.scene.name;
                            allGroups.Add(tempVolumeList[i]);
                        }
                    }
                }
                else
                {
                    tempVolumeList.AddRange(Object.FindObjectsOfType<MagicLightProbes>());

                    for (int i = 0; i < tempVolumeList.Count; i++)
                    {
                        if (tempVolumeList[i].isInPrefab)
                        {
                            if (!allGroups.Find(item => item.prefabUID == tempVolumeList[i].prefabUID))
                            {
                                allGroups.Add(tempVolumeList[i]);
                            }
                        }
                        else if (!tempVolumeList[i].isInPrefab)
                        {
                            allGroups.Add(tempVolumeList[i]);
                        }
                    }
            ...
            switch (selectedTab)
            {
                ...
                case 1:
                    ...
                    if (GUILayout.Button("Calculate All Volumes"))
                    {
                        ...
                        for (int i = 0; i < allGroups.Count; i++)
                        {
                            if (!allGroups[i].useVolumeBottom && allGroups[i].groundAndFloorObjects.Count == 0)
                            {
                        ...
                        for (int i = 0; i < allGroups.Count; i++)
                        {
                            if (allGroups[i].isInPrefab && PrefabStageUtility.GetCurrentPrefabStage() == null)
                            {
                                continue;    
                            }
                    ...
                    for (int i = 0; i < allGroups.Count; i++)
                    {
                        if (allGroups[i] != null)
                        {
                            ...
                            string prefabMark = string.IsNullOrEmpty(allGroups[i].prefabRootName)
                                ? ""
                                : " [" + allGroups[i].prefabRootName + "]";
                            ...
                            {
                                GUILayout.Label("NOT CALCULATED", managerWindowProcessLabelStyle);
                            }
                            ...
                            { 
                                GUILayout.FlexibleSpace();
                                GUILayout.Label(allGroups[i].gameObject.name + prefabMark, managerWindowCaptionStyle);
                                GUILayout.FlexibleSpace();

                                GUILayout.BeginVertical();

                                if (currentPrefabStage == null && allGroups[i].isInPrefab)
                                {
                                    GUI.enabled = false;
                                    removeButtonContent = new GUIContent("Remove",
                                        "Removing a volume is only possible from the prefab edit mode.");
                                }
                                else
                                {
                                    removeButtonContent = new GUIContent("Remove",
                                ...
                                if (GUILayout.Button(removeButtonContent, managerWindowTopPanelButtonStyle))
                                {
                            ...
                            allGroups[i].showOptionsInManagerWindow = 
                                EditorGUILayout.Foldout(allGroups[i].showOptionsInManagerWindow, 
                                    allGroups[i].name + prefabMark + " Options", 
                                    true,
