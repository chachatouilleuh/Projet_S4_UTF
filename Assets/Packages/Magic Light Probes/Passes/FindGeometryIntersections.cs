...
namespace MagicLightProbes
{
    ...
    public class FindGeometryIntersections
    {
        ...
        public IEnumerator ExecutePass(MagicLightProbes parent, int volumePartIndex, float cornersDetectionThreshold, MLPVolume currentVolume = null, bool realtimeEditing = false)
        {
            ...
            string dirPath = parent.assetEditorPath + "/Scene Data/" + UnityEngine.SceneManagement.SceneManager.GetActiveScene().name + "/" + parent.name + "/GeometryIntersectionsData";
                ...
                for (int i = 0; i < parent.tmpSharedPointsArray.Count; i++)
                {
                    ...
                    foreach (var ray in checkRays)
                    {
                        if (parent.RaycastPhysicsScene(
                            ray.origin,
                            ray.direction,
                            cornersDetectionThreshold + 0.1f,
                            out hitInfo))
                        {
                    ...
                    if (results.Count > 1)
                    {
                        ...
                        if (parent.RaycastPhysicsScene(
                            rayToPoint.origin,
                            rayToPoint.direction,
                            cornersDetectionThreshold * 2,
                            out hitInfo))
                        {
                            newPoint.position = hitInfo.point;
                            newPoint.position -= avaragedDirection * parent.distanceFromNearbyGeometry;

                            if (!parent.CheckIfInside(currentVolume.transform, newPoint.position))
                            {
                                continue;
                            }
                        ...
                        else if (parent.tmpGeometryCollisionPoints.Contains(parent.tmpSharedPointsArray[i]))
                        {
                            parent.tmpSharedPointsArray.RemoveAt(i);
                        }
                    }
